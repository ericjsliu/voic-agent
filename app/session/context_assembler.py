...
class ContextAssembler:
    ...
    def __init__(
        ...
        p2_memory_service: P2MemoryService = None
    ):
        self.memory_store = memory_store
        self.entity_buffer = entity_buffer
        self.p2_memory_service = p2_memory_service
        if not self.p2_memory_service:
            self.p2_memory_service = P2MemoryService(enable_vector=True)
    ...
    async def _get_memory_slice(
        ...
        单一P2路径：仅使用向量召回（≤5条，≤300 token）
        """
        memory_slice = {}
        
        # 向量召回相关记忆（异步，失败不阻塞）
        if driver_id and self.p2_memory_service:
            try:
                ...
                if query:
                    ...
                    if memories:
                        memory_slice["relevant_memories"] = [
                            {
                                "content": m["content"],
                                "category": m["category"]
                            }
                        ...
                        print(f"[ContextAssembler] Injected {len(memories)} memories")
            except Exception as e:
                # 失败降级：不注入，不阻塞主对话
                print(f"[ContextAssembler] Memory recall failed (soft): {e}")
