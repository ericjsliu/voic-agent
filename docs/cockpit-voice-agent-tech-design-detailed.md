...
| 能力 | 设计 |
|------|------|
| 被动语义触发 | PRD v1.27修正: 回合中入队候选到Redis，scheduled batch job（nightly/每N小时）扫描PG task records批量打分提取；长期性/稳定性/个人属性过阈值 → 按类目策略写入或提案 |
...
| 被动 | PRD v1.27修正: 对话中入队候选到Redis，scheduled batch job扫描PG task records批量打分：长期性 × 稳定性 × 个人属性 ≥ 阈值 | 提取 → 门闩 → 入库 |
