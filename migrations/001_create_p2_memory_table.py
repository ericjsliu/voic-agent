...
def create_p2_memory_table():
    """创建P2长期记忆表（1024维向量，用户锁定text-embedding-v3）"""
    ...
    print("[Migration] Created long_term_memory_p2 table (1024-dim vectors)")
    ...
    with _engine.connect() as conn:
        ...
        try:
            ...
            """))
            conn.commit()
            print("[Migration] Created vector index (ivfflat, 1024-dim)")
        except Exception as e:
            print(f"[Migration] Vector index creation skipped (may need more data): {e}")
    
    print("[Migration] Migration complete!")
    print("[Migration] Note: Vector dimension is 1024 (user-locked text-embedding-v3)")
